#!/usr/bin/env python
# ulimit -n 4096
# nohup python main.py --inference --model FlowNet2 --save_flow
...
if __name__ == '__main__':
    top_path = '/home/yhzn/ws/crop_image'
    crop_image_path = '/home/yhzn/ws/flow_result'
    os_dir_list = []
    read_video(top_path, os_dir_list)
    os_dir_list.sort()
    sort_file = 'sort_result.txt'
    with open(sort_file,"w") as f:
        for i in os_dir_list:
            f.write(i)
            f.write('\n')

    all_video_cnt = len(os_dir_list)
    # print(os_dir_list[18])
    cur_video_cnt = 0
    for each_video_num in range(len(os_dir_list)):
        each_video = os_dir_list[each_video_num]

        #if each_video.split('/')[-1] != 'clip_000007.mp4':
         #   continue

        # print('//////////////////')
        print("each video = ", each_video)
        file_list = os.listdir(each_video)
        if len(file_list) == 0:
            cur_video_cnt += 1
            print('File is empty!')
            print("[%d / %d]" % (cur_video_cnt, all_video_cnt))
            continue
        ...
        # 需要修改
        parser.add_argument('--resume', default='./model/FlowNet2_checkpoint.pth.tar', type=str,
        ...
        tools.add_arguments_for_module(parser, datasets, argument_for_class='training_dataset',
                                       default='MpiSintelFinal',
                                       skip_params=['is_cropped'],
                                       parameter_defaults={'root': './MPI-Sintel/flow/training'})

        tools.add_arguments_for_module(parser, datasets, argument_for_class='validation_dataset',
        ...
        with tools.TimerBlock("Initializing Datasets") as block:
            ...
            if exists(args.validation_dataset_root):
                ...
                validation_loader = DataLoader(validation_dataset, batch_size=args.effective_batch_size, shuffle=False,
                                               **gpuargs)
            args.inference_dataset_root = each_video
        ...
        def train(args, epoch, start_iteration, data_loader, model, optimizer, logger, is_validate=False, offset=0):
            ...
            if is_validate:
                ...
                progress = tqdm(tools.IteratorTimer(data_loader), ncols=100,
                                total=np.minimum(len(data_loader), args.validation_n_batches), leave=True,
                                position=offset,
                                desc=title)
            else:
                model.train()
            ...
            for batch_idx, (data, target) in enumerate(progress):
                ...
                if ((((global_iteration + 1) % args.log_frequency) == 0 and not is_validate) or
                    ...
                    for i, key in enumerate(loss_labels):
                        logger.add_scalar(
                            'average batch ' + str(key), all_losses[:, i].mean(), global_iteration)
        ...
        for epoch in progress:
            ...
            train_logger.add_scalar(
                'seconds per epoch', progress._time() - last_epoch_time, epoch)
            last_epoch_time = progress._time()
        cur_video_cnt += 1
        print("[%d / %d]" % (cur_video_cnt, all_video_cnt))
        print("\n")
