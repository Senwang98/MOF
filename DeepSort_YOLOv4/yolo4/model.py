...
def resblock_body(x, num_filters, num_blocks, all_narrow=True):
    ...
    preconv1 = DarknetConv2D_BN_Mish(
        num_filters, (3, 3), strides=(2, 2))(preconv1)
...
def yolo4_body(inputs, num_anchors, num_classes):
    ...
    maxpool1 = MaxPooling2D(pool_size=(
        13, 13), strides=(1, 1), padding='same')(y19)
    maxpool2 = MaxPooling2D(pool_size=(
        9, 9), strides=(1, 1), padding='same')(y19)
    maxpool3 = MaxPooling2D(pool_size=(
        5, 5), strides=(1, 1), padding='same')(y19)
    y19 = Concatenate()([maxpool1, maxpool2, maxpool3, y19])
    y19 = DarknetConv2D_BN_Leaky(512, (1, 1))(y19)
    y19 = DarknetConv2D_BN_Leaky(1024, (3, 3))(y19)
    y19 = DarknetConv2D_BN_Leaky(512, (1, 1))(y19)

    y19_upsample = compose(DarknetConv2D_BN_Leaky(
        256, (1, 1)), UpSampling2D(2))(y19)
    ...
    y38_upsample = compose(DarknetConv2D_BN_Leaky(
        128, (1, 1)), UpSampling2D(2))(y38)
    ...
    y76_downsample = DarknetConv2D_BN_Leaky(
        256, (3, 3), strides=(2, 2))(y76_downsample)
    ...
    y38_downsample = DarknetConv2D_BN_Leaky(
        512, (3, 3), strides=(2, 2))(y38_downsample)
...
def box_giou(b1, b2):
    ...
    giou = iou - 1.0 * (enclose_area - union_area) / \
        (enclose_area + K.epsilon())
...
def yolo4_loss(args, anchors, num_classes, ignore_thresh=.5, label_smoothing=0, use_focal_loss=False, use_focal_obj_loss=False, use_softmax_loss=False, use_giou_loss=False, use_diou_loss=False):
    ...
    for l in range(num_layers):
        ...
        if label_smoothing:
            true_class_probs = _smooth_labels(
                true_class_probs, label_smoothing)
        ...
        if use_focal_loss:
            ...
            if use_softmax_loss:
                ...
                class_loss = object_mask * K.expand_dims(K.categorical_crossentropy(
                    true_class_probs, raw_pred[..., 5:], from_logits=True), axis=-1)
            else:
