...
class LocationDatasetJAAD(Dataset):
    """
        根据filename的帧提取包含其在内的 NUM_FLOW_FRAMES 帧光流
        返回 {'flow_stack': flow_stack, 'labels': label, 'filenames': filename}
        flowstack: (256, 256, 18)
        labels: 需要拟合的误差GT
        filename: 含义不变
    """
...
def test(model, device, test_loader, loss_function):
    ...
    print('Total index = ', cnt)
    print('all_outputs_15 size = ', all_outputs_15.shape)
...
def main(args):
    ...
    # print('trainset flow_stack size = ', trainset['flow_stack'].shape)
    # print('valset flow_stack size = ', valset['flow_stack'].shape)
...
"""
    for fold in [1, 2, 3, 4, 5]:
        ...
        print("all_outputs size = ", all_outputs.shape)
        print('all_targets size = ', all_targets.shape)

        # Save the model
        torch.save(best_model.state_dict(),
                   model_save_path + 'rn18_flow_css_' + str(NUM_FLOW_FRAMES) + 'stack_fold_' + str(
                       fold) + '_pretrained-' + str(pretrained) + '_disp.weights')

        # Save the predictions and the targets
        np.save('./predictions_rn18_flow_css_' + str(NUM_FLOW_FRAMES) + 'stack_jaad_fold_' + str(
            fold) + 'pretrained-' + str(pretrained) + '_disp.npy', all_outputs)
        np.save(
            './targets_rn18_flow_css_' + str(NUM_FLOW_FRAMES) + 'stack_jaad_fold_' + str(fold) + 'pretrained-' + str(
                pretrained) + '_disp.npy', all_targets)

        # Save the results
        result = {'NUM_FLOW_FRAMES': NUM_FLOW_FRAMES, 'fold': fold, 'val_mse': best_MSE, 'val_fde': best_FDE,
                  'test_mse_5': test_mse_5, 'test_fde_5': test_fde_5, 'test_mse_10': test_mse_10,
