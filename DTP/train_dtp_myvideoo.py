...
def get_modified_resnet(NUM_FLOW_FRAMES):
    ...
    model.conv1 = nn.Conv2d(NUM_FLOW_FRAMES * 2, 64,
...
class LocationDatasetBDD(Dataset):
    def __init__(self, filename, root_dir, img_root, transform, NUM_FLOW_FRAMES, proportion=100):
        ...
        unique_filenames = unique_filenames[0:int(
            len(unique_filenames) * proportion / 100)]
    ...
    def __getitem__(self, idx):
        ...
        for frame in range(frame_num - (NUM_FLOW_FRAMES - 1) * 3, frame_num + 1, 3):
            ...
            flow_stack[:, :,int((frame - (frame_num - (NUM_FLOW_FRAMES - 1) * 3)) // 3 * 2)] = hor_flow
            flow_stack[:, :,int((frame - (frame_num - (NUM_FLOW_FRAMES - 1) * 3)) // 3 * 2 + 1)] = ver_flow
...
def train(model, device, train_loader, optimizer, epoch, loss_function):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        flow, targets = data['flow_stack'].to(
            device), data['labels'].to(device)
        ...
        all_outputs_5 = np.append(
            all_outputs_5, output_5.detach().cpu().numpy())
        all_targets_5 = np.append(
            all_targets_5, targets_5.detach().cpu().numpy())
        all_outputs_10 = np.append(
            all_outputs_10, output_10.detach().cpu().numpy())
        all_targets_10 = np.append(
            all_targets_10, targets_10.detach().cpu().numpy())
        all_outputs_15 = np.append(
            all_outputs_15, output_15.detach().cpu().numpy())
        all_targets_15 = np.append(
            all_targets_15, targets_15.detach().cpu().numpy())
    ...
    print(
        'Train Epoch: {} [{}/{} ({:.0f}%)] \tMSE@5: {:.0f} \tFDE@5: {:.0f} \tMSE@10: {:.0f} \tFDE@10: {:.0f} \tMSE@15: {:.0f} \tFDE@15: {:.0f}'.format(
...
def test(model, device, test_loader, loss_function):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(test_loader):
            ...
            targets_5 = torch.cat((targets[:, 0:5], targets[:, 15:20]), dim=1)
            targets_10 = torch.cat(
                (targets[:, 0:10], targets[:, 15:25]), dim=1)
            targets_15 = targets
    ...
    print(
        'Validation: \t\t\t\tMSE@5: {:.0f} \tFDE@5: {:.0f} \tMSE@10: {:.0f} \tFDE@10: {:.0f} \tMSE@15: {:.0f} \tFDE@15: {:.0f}'.format(
...
def main(args):
    ...
    if args.detector == 'yolo':
        # img_root = './data/optical_flow/'
        img_root = '/home/yhzn/ws/flow_result/'
    else:
    ...
    weight_decay = 1e-2
    NUM_FLOW_FRAMES = 9
    training_proportion = 100  #  How much of the dataset to use? 100 = 100 percent
    ...
    print(args.detector + '_myvideo_val.pkl')

    try:
        testset = LocationDatasetBDD(filename='myvideo_val_' + args.detector + '.pkl',
                                     root_dir=load_path, transform=transform_val, img_root=img_root,
                                     NUM_FLOW_FRAMES=NUM_FLOW_FRAMES)
        test_loader = torch.utils.data.DataLoader(testset, batch_size=batch_size,
                                                  shuffle=False, num_workers=num_workers)

        trainset = LocationDatasetBDD(filename='myvideo_train_' + args.detector + '.pkl',
                                      root_dir=load_path, transform=transform_train, img_root=img_root,
                                      NUM_FLOW_FRAMES=NUM_FLOW_FRAMES, proportion=training_proportion)
        train_loader = torch.utils.data.DataLoader(trainset, batch_size=batch_size,
                                                   shuffle=True, num_workers=num_workers)

        valset = LocationDatasetBDD(filename='myvideo_val_' + args.detector + '.pkl',
                                    root_dir=load_path, transform=transform_val, img_root=img_root,
                                    NUM_FLOW_FRAMES=NUM_FLOW_FRAMES)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if MSE_15 < best_MSE:
            ...
            best_model = copy.deepcopy(model)
            best_FDE = FDE_15
            torch.save(best_model.state_dict(), model_save_path + args.detector + '_rn50_myvideo_flow_css_' + str(
                NUM_FLOW_FRAMES) + 'stack_training_proportion_' + str(training_proportion) + '_shuffled_disp.weights')
        print(epoch)
    ...
    # Save the model
    torch.save(best_model.state_dict(), model_save_path + args.detector + 'myvideo_rn50_flow_css_' + str(
        NUM_FLOW_FRAMES) + 'stack_training_proportion_' + str(training_proportion) + '_shuffled_disp.weights')

    # Save the predictions and the targets
    np.save('./' + args.detector + '_predictions_rn50_flow_css_' + str(
        NUM_FLOW_FRAMES) + 'stack_myvideo_training_proportion_' + str(training_proportion) + '_shuffled_disp.npy',
        all_outputs)
    np.save('./' + args.detector + '_targets_rn50_flow_css_' + str(NUM_FLOW_FRAMES) + 'stack_myvideo__shuffled_disp.npy',
            all_targets)

    # Save the results
    result = {'NUM_FLOW_FRAMES': NUM_FLOW_FRAMES, 'training_proportion': training_proportion, 'val_mse': best_MSE,
              'val_fde': best_FDE, 'test_mse_5': test_mse_5, 'test_fde_5': test_fde_5, 'test_mse_10': test_mse_10,
              'test_fde_10': test_fde_10, 'test_mse_15': test_mse_15, 'test_fde_15': test_fde_15}
    results = results.append(result, ignore_index=True)
    results.to_csv('./' + args.detector +
                   '_results_rn50_myvideo.csv', index=False)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--detector', '-d', help="Use detections from 'yolo' or 'faster-rcnn'", type=str,
                        default='yolo')
