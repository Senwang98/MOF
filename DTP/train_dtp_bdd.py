...
def main(args):
    ...
    for epoch in range(1, epochs + 1):
        ...
        if MSE_15 < best_MSE:
            ...
            best_model = copy.deepcopy(model)
            best_FDE = FDE_15
            torch.save(best_model.state_dict(), model_save_path + args.detector + '_rn18_bdd10k_flow_css_' + str(
    ...
    # Save the model
    torch.save(best_model.state_dict(), model_save_path + args.detector + 'bdd10k_rn18_flow_css_' + str(
        NUM_FLOW_FRAMES) + 'stack_training_proportion_' + str(training_proportion) + '_shuffled_disp.weights')

    # Save the predictions and the targets
    np.save('./' + args.detector + '_predictions_rn18_flow_css_' + str(
        NUM_FLOW_FRAMES) + 'stack_bdd10k_training_proportion_' + str(training_proportion) + '_shuffled_disp.npy',
            all_outputs)
    np.save('./' + args.detector + '_targets_rn18_flow_css_' + str(NUM_FLOW_FRAMES) + 'stack_bdd10k__shuffled_disp.npy',
            all_targets)
