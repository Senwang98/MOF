...
'''
将所有的检测结果依次执行如下操作：
    剔除遮挡物体、转化15FPS、调整分辨率720P、增加[labeled, past_x, past_y, future_x, future_y]
    剔除小物体、设置[labeled, past_x, past_y, future_x, future_y]具体的数值。
labeled表示当前帧前面有10帧跟踪帧、后面有15帧跟踪帧。
past_x/y为数组，只有在labeled=1的帧才有效，保存过去的检测框位置。
feture数组同理。
'''
import pandas as pd
import numpy as np
...
SIXTY_FPS_VIDEOS = ['video0028.mp4', 'video0037.mp4', 'video0038.mp4', 'video0039.mp4', 'video0040.mp4',
                    'video0041.mp4', 'video0052.mp4', 'video0053.mp4']
LOW_RES_VIDEOS = ['video0061.mp4', 'video0062.mp4', 'video0063.mp4', 'video0064.mp4', 'video0065.mp4', 'video0066.mp4',
                  'video0067.mp4', 'video0068.mp4', 'video0069.mp4', 'video0070.mp4']
SAVE_PATH = '../data/'
...
for rownum in range(1, len(boxes[1:])):
    ...
    if boxes.iloc[rownum]['detection_length'] >= MIN_DETECTION_LENGTH:
        ...
        past_x = (boxes.iloc[rownum - MIN_DETECTION_LENGTH + 1:rownum - MIN_DETECTION_LENGTH + MIN_LENGTH_PAST + 1][
            'Mid_x']).values
        past_y = (boxes.iloc[rownum - MIN_DETECTION_LENGTH + 1:rownum - MIN_DETECTION_LENGTH + MIN_LENGTH_PAST + 1][
            'Mid_y']).values
...
boxes.to_csv(SAVE_PATH + 'jaad_location_features.csv')
